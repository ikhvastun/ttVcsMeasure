...
void showHist(TVirtualPad* c1, DistribsAll & distribs, string title, string titleX, string titleY, double num, TLegend *leg, bool plotInLog = false, bool normalizedToData = false, double lumi = 35.9){
    ...
    for(unsigned int i = 0; i < numberOfSyst; i++){
        ...
        //stackUncUp[i] = (TH1D*)(distribs.stack.GetStack()->Last())->Clone(Form("stackUncUp_%d", i));
        //stackUncDown[i] = (TH1D*)(distribs.stack.GetStack()->Last())->Clone(Form("stackUncDown_%d", i));
...
void showHist(TVirtualPad* c1, DistribsAll & distribs, string title, string titleX, string titleY, double num, TLegend *leg, bool plotInLog = false, bool normalizedToData = false, double lumi = 35.9){   
    ...
    TH1D *histCopyUnc = (TH1D*)distribs.vectorHisto[dataSample].Clone("histCopyUnc");
    //TH1D *histCopyUnc =  (TH1D*)(distribs.stack.GetStack()->Last())->Clone(Form("histCopyUnc"));
    histCopyUnc->Reset();
    // stack of MC with varied up and down of 3 different types of uncertainties
    TH1D *stackUncUp[2];
    TH1D *stackUncDown[2];
    ...
    for(unsigned int i = 0; i < 2; i++){
        ...
        for(unsigned int j = distribs.vectorHistoUncUp.size()-1; j != 0; j--){
            stackUncUp[i]->Add((TH1D*)&distribs.vectorHistoUncUp[j].unc[i]);
            stackUncDown[i]->Add((TH1D*)&distribs.vectorHistoUncUp[j].unc[i]);
        }
    } 

    for(unsigned int i = 0; i < histCopyUnc->GetNbinsX(); i++){
        // content in particular bin in stack
        double stackBinContent = ((TH1D*)distribs.stack.GetStack()->Last())->GetBinContent(i+1);
        double stackBinError = ((TH1D*)distribs.stack.GetStack()->Last())->GetBinError(i+1);

        stackCopy->SetBinError(i+1, (stackBinContent != 0. ? stackBinError / stackBinContent : 1.));
        double err = TMath::Power(stackCopy->GetBinError(i+1), 2);
        cout << "stat err is: " << TMath::Sqrt(err) << endl;

        for(unsigned int j = 0; j < 2; j++){
            // consider largest deviation between the upward and downward variations
            cout << "central value: " << stackBinContent << "; upward fluct: " << stackUncUp[j]->GetBinContent(i+1) << "; downward fluct: " << stackUncDown[j]->GetBinContent(i+1) << endl;
            err += TMath::Power(TMath::Max(stackUncUp[j]->GetBinContent(i+1) - stackBinContent, stackBinContent - stackUncDown[j]->GetBinContent(i+1)) / stackBinContent, 2);
            cout << "unc after applying " << j << " syst: " << TMath::Sqrt(err) << endl;
            histCopyUnc->SetBinContent(i+1, 1.);
            // if uncertainty is greater than 100% consider 100% uncertainty
            histCopyUnc->SetBinError(i+1, TMath::Sqrt(err) > 1 ? 1. : TMath::Sqrt(err));
        }
    }

    if(titleX=="monetplot"){
      stackCopy->GetXaxis()->SetTitle("");
      stackCopy->GetXaxis()->SetTitleOffset(0.6);
    ...
    if(titleX == "flavour"){
       stackCopy->GetXaxis()->SetTitle("");
       stackCopy->GetXaxis()->SetTitleOffset(0.6);
       stackCopy->GetXaxis()->SetLabelSize(0.3);
    }
    ...
    // only stat unc
    for(unsigned int i = 0; i < histCopyUnc->GetNbinsX(); i++){
        cout << "unc in bin " << i+1 << " is: " << histCopyUnc->GetBinError(i+1) << endl;
    }

    histCopyUnc->SetFillStyle(1001);
    histCopyUnc->SetFillColor(kBlue - 4);
    histCopyUnc->SetMarkerStyle(1);
    histCopyUnc->Draw("e2same");

    stackCopy->SetFillStyle(1001);
    stackCopy->SetFillColor(kOrange - 4);
    stackCopy->SetMarkerStyle(1);
    //stackCopy->Draw("e2same");
