...
void sigmaPerp(){
	...
	TFile *_file0 = TFile::Open("sigmaPerpEl.root");
	TFile *_file1 = TFile::Open("sigmaPerpMu.root");
	TH1D* h1 = (TH1D*) _file0->Get("resChoice_1_0");
	TH1D* h2 = (TH1D*) _file1->Get("resChoice_1_0");

	TFile *_file2 = TFile::Open("scaleEl.root");
	TFile *_file3 = TFile::Open("scaleMu.root");

	TH1D* h3 = (TH1D*) _file2->Get("scaleChoice_1");
	TH1D* h4 = (TH1D*) _file3->Get("scaleChoice_1");

	h1->SetTitle("");
	h1->GetYaxis()->SetTitle("#sigma( u_{#perp}  ) [GeV]");
