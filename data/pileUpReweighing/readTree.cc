...
void readTree()
{
    ...
    TFile *file0 = TFile::Open("MyDataPileupHistogram_GH.root");
    ...
    TH1F *htrue = new TH1F("htrue", "htrue", 60, 0, 60);
    htrue->Sumw2();

    TFile * hfile = new TFile("/Users/illiakhvastunov/Desktop/CERN/MCsamples/80X/ttW_ReReco_NewMC/ttbar_2l.root","read");
        
    hfile->cd("FakeElectrons");
    TTree* inputTreeFake = static_cast<TTree*>(hfile->Get("FakeElectrons/fakeTree"));
    Long64_t nEntries = inputTreeFake->GetEntries();
    Double_t        _n_MCTruth_PV;

    inputTreeFake->SetBranchAddress("_n_MCTruth_PV", &_n_MCTruth_PV);
    std::cout<<"Entries in ttbar"<<" "<<nEntries<<std::endl;
    ...
    */
    ...
    TFile *file = TFile::Open("puWeights_GH.root","RECREATE");
    h3->Draw();
    h3->Write();
    file->Close();
